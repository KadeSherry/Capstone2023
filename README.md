...
### Creating a Docker Container to run Ansible
This section explains how to create a Docker container that runs the Ansible dynamic inventory and playbooks. This is so you can have you Ansible materials segmented from the container that Nautobot is running on, but still be able to query Nautobot for the required information like hostname and IP addresses.
-First, install Docker onto your machine with the below commands, starting by updating the machine you’re putting Docker on:
```bash
sudo apt-get update
sudo apt install docker.io -y
```
-Verify instalation by runing the bellow command, which displays any active version:
```bash
docker --version
```
-Next, create a Docker container image for building your Docker container. The below code is what we chose to include in our file, but your requirments may necessitate differences:
```bash
FROM ubuntu:18.04	
...
```
-Build the image for Docker using the directories from your Docker file. Use the below command to build the image.
```bash
docker build -t ansible:host .
```
-Then run your Docker you created in interactive mode by entering the below command.
```bash
docker run -it ansible
```
-To switch to the Ansible container enter the below command. The container ID that is included in the command can be changed to a different one or the container name like Ansible for example.
```bash
...
### Ansible Files
Please see 'ansible' folder to see what code you will need.

### Running the Files
After you have created all the files that you will need to run Ansible playbooks and have collected the necessary information, use the following commands to run them:
Note: Before you can run these files to create an inventory and change configuration you need to launch the container that holds the lab topology. If you haven’t launched that yet at this point run this command making sure to remove the quotation marks when running the command:
```bash
sudo clab deploy -t containerlab/topologies/L2LS/L2LS.yaml –reconfigure”
```
...
sudo docker exec -it ae03d02f6e81 /bin/bash
```
-Change to the directory that contains your ansible files
```bash
cd Ansible/AnsiblePlays/
```
-First run the GetInv.yml file to query Nautobot for device information like hostname and IP address
```bash
ansible-inventory --list GetInv.yml > inventory.json
```
-After you have entered that command run the python file extractIP.py. This file will take the JSON output from the previous command and format it into an inventory.ini file for Ansible to use.
```bash
python3 extractIP.py
```
-Now that you have queried Nautobot for the information you need to connect and formatted it so it can be used by Ansible you can start running your playbooks with the following command:.
```bash
ansible-playbook -u admin -k playbook.yml
```
-Note: Replace “playbook.yml” with the playbooks that you have created. For example, ansible-playbook -u admin -k showRun.yml. Running this command will also prompt the user to enter the device password to login to the device. The -k option in this command will prompt you to enter the password for the device before the Ansible play can be run.
